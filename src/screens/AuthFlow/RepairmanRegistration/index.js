...
import { Text, TouchableOpacity, StatusBar, TextInput, View, ScrollView, ActivityIndicator } from 'react-native';
import { responsiveHeight, responsiveWidth } from 'react-native-responsive-dimensions';
import { styles } from './style';
import Toast from 'react-native-simple-toast';

const RepairmanRegistration = props => {
  ...
  const [cnPH, setcnPH] = useState('Company Name');
  const [caPH, setcaPH] = useState('Company Address');
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.Infoview}>
        <Text style={styles.mainTxt}>Repairman Resgistration</Text>
        <Text style={styles.mainSubTxt}>{'please fill in the form below to register \nyourself as a repairman...'} </Text>
      </View>
      <View style={styles.viewInput}>
        ...
        <View style={{ alignItems: 'center' }}>
          <TextInput
            ...
            onFocus={() => setcnPH('')}
            onBlur={() => (companyname === '' ? setcnPH('Company Name') : null)}
          />
        </View>
        {!companynameerror ? null : <Text style={{ color: 'red', marginLeft: responsiveWidth(6) }}>{companynameerror}</Text>}
        <View style={{ alignItems: 'center' }}>
          <TextInput
            style={styles.txtInput}
            placeholder={caPH}
            placeholderTextColor="black"
            onChangeText={companyadd => setcompanyadd(companyadd)}
            onFocus={() => setcaPH('')}
            onBlur={() => (companyadd === '' ? setcaPH('Company Address') : null)}
          />
        </View>
        {!companyadderror ? null : <Text style={{ color: 'red', marginLeft: responsiveWidth(6) }}>{companyadderror}</Text>}
        <View style={{ alignItems: 'center' }}>
          <TextInput
            style={styles.txtInput}
            placeholder={emailPH}
      ...
      <View
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            if (companyadd === '') {
              setcompanyadderror('Please enter your company address');
              return;
            }
            if (email === '') {
      ...
      <View style={styles.endText}>
        ...
        <TouchableOpacity onPress={() => _onLogin()}>
...
export default RepairmanRegistration;
